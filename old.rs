...
use shiromana_rs::misc::{Error, Uuid};
    ...
    let server_msg = async || {
        match api_name {
            ...
            "create_series" => {
                ...
                match library.create_series(
                    qs_get_string("caption").unwrap_or("Series".to_string()),
                    qs_get_string("comment"),
                ) {
                    Ok(v) => ServerMessage {
                        library_uuid: Some(uuid),
            ...
            "add_to_series" => {
                ...
                let is_sorted = qs.has("sorted");
                match library.add_to_series(id, &series_uuid, no, is_sorted) {
                    Ok(_) => ServerMessage::default(),
                    Err(e) => wrapped_from_single_error(
                        "series",
            ...
            "remove_from_series" => {
                ...
                match library.remove_from_series(id, &uuid) {
                    Ok(_) => ServerMessage::default(),
                    Err(e) => wrapped_from_single_error(
                        "series",
            ...
            "update_series_no" => {
                ...
                match library.update_series_no(id, &uuid, no, insert) {
                    Ok(_) => ServerMessage::default(),
                    Err(e) => wrapped_from_single_error(
                        "series",
            ...
            "trim_series_no" => {
                ...
                match library.trim_series_no(&series_uuid) {
                    ...
                    Err(e) => wrapped_from_single_error(
                        "series",
                        format!(
                            "Failed trim series no for a series `{}`: {}.",
                            series_uuid, e
                        ),
                    ),
                }
            ...
            "create_tag" => {
                ...
                match library.create_tag(
                    ...
                    Err(e) => {
                        wrapped_from_single_error("series", format!("Failed to create tag: {}.", e))
                    }
                }
            }
            "delete_tag" => {
                let uuid = extract_option_of_param!("uuid", library_uuid);
                let tag_uuid = qs_get_and_parse!("tag_uuid", Uuid);
                let mut opened_libraries = context.opened_libraries.lock().await;
                let mut library = opened_libraries.get_mut(&uuid);
                let mut library = match library {
                    Some(v) => v,
                ...
                match library.delete_tag(tag_uuid) {
                    Ok(_) => ServerMessage::default(),
                    Err(e) => wrapped_from_single_error(
                        "series",
                        format!("Failed to delete tag `{}`: {}.", tag_uuid, e),
                    ),
                }
            }
            "add_tag" => {
                ...
                match library.add_tag(id, tag_uuid) {
                    Ok(_) => ServerMessage::default(),
                    Err(e) => wrapped_from_single_error(
                        "series",
                        format!("Failed to add media to tag `{}`: {}.", tag_uuid, e),
                    ),
                }
            }
            "remove_tag" => {
                ...
                match library.remove_tag(id, tag_uuid) {
                    Ok(_) => ServerMessage::default(),
                    Err(e) => wrapped_from_single_error(
                        "series",
                        format!("Failed to remove media from tag `{}`: {}.", tag_uuid, e),
                    ),
                }
            }
            "make_thumbnail" => {
                ...
                /*
                let match library.remove_tag(id, tag_uuid) {
                    Ok(_) => ServerMessage::default(),
                    Err(e) => wrapped_from_single_error(
                        "series",
                        format!("Failed to remove media from tag `{}`: {}.", tag_uuid, e),
                ...
                let recv = library.make_thumbnail(id);
                match recv.recv() {
                    Ok(v) => match v {
                        Ok(v) => ServerMessage {
                            value: Some(base64::encode(v)),
                            ..ServerMessage::default()
                        },
                        Err(Error::NoThumbnail) => ServerMessage::default(),
                        Err(err) => wrapped_from_single_error(
                            "make_thumbnail",
                            format!("Thumb error while making thumbnail. {}", err.to_string()),
                        ),
                    },
                    Err(err) => wrapped_from_single_error(
                        "make_thumbnail",
                        format!("Recv error while making thumbnail. {}", err.to_string()),
                    ),
                }
            }
            _ => ServerMessage {
